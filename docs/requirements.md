...
開発は私用 PC + 個人 Slack ワークスペースで行い、会社 PC には「移植」でなく
**同じ手順で再現**する。環境依存物はすべて `.env` に隔離する:

| 要素 | 方針 |
|---|---|
| Slack App 構成 | **App Manifest (YAML) をリポジトリに含める**。会社ワークスペースでは "Create from manifest" で同一構成を再作成 |
| Slack トークン | ワークスペースごとに発行し `.env` へ (コミットしない) |
| LLM | `claude -p` は各マシンのログイン済みシートを使う (設定不要) |
| データ (SQLite) | **意図的に移さない**。私用の文脈と会社の文脈は混ぜない |
| 常駐設定 | setup スクリプトで再現 |

会社側で必要な承認は 2 つ: ① Slack 管理者のアプリ承認 (Socket Mode +
最小スコープの manifest がそのまま申請資料になる)、② 社給機での実行ポリシー
(Claude Code 許可済みの会社なら通常問題なし)。
