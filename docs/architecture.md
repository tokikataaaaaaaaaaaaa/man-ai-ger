...
`manaiger init` は初回起動前の bootstrap を担当する。設定は「プロセス起動前に必要な設定」と「起動後に DB で変えられる運用設定」に分ける。

設定読み込み順:

1. プロセス環境変数
2. カレントディレクトリの `.env` (開発・明示的なローカル override)
3. `~/.manaiger/.env` (インストール済み CLI の bootstrap 設定)
4. `MANAIGER_HOME/.env` (bootstrap で別 home が指定された場合の home-local 設定)

既に存在する環境変数は `.env` で上書きしない。後に読む `.env` は、前に読んだ値を上書きしない。
...
- `SLACK_BOT_TOKEN`
- `SLACK_APP_TOKEN`
- `MANAIGER_DASHBOARD_PORT`
- `MANAIGER_CODEX_PATH`
- `MANAIGER_CODEX_MODEL`

DB settings に保存する運用設定:
...
Dashboard `/settings` は DB settings の運用設定だけを更新する。起動前に必要な設定は read-only 表示に留める。

MVP の connector は Slack のみに固定する。Atlassian / Discord / Teams / GitHub などの追加 connector や MCP 登録は、Integration registry を導入する将来バックログとして扱う。
...
- MVP で event ingestion を持つ外部 connector は Slack のみ
...
### 4.3 将来 connector backlog

Slack 以外の連携は MVP では実装しない。追加する場合は、connector ごとに次を定義してから Integration registry に登録する。

- event ingestion か read-only context source か
- 必要 scope と secret の保存先
- `doctor` の確認項目
- Dashboard Service に表示する状態
- データ経路ガードの許可範囲
...
```text
manaiger init     : 初回起動に必要な bootstrap 設定と DB を作る
...
- bootstrap `.env` を作成または更新する
- `MANAIGER_HOME` に SQLite DB を初期化する
- working hours / interaction timing は DB settings に保存する

安全性:

- 既存設定は確認なしに消さない
- Slack token は標準出力に全文表示しない
- init 中に Slack / Codex へデータ送信しない
- 最後に `manaiger doctor` と `manaiger start` を案内する
